...
class MultiTimeSlotTest extends TestCase
{
    ...
    /** @test */
    public function a_multi_available_slot_can_be_retrieved()
    {
    ...
    /** @test */
    public function a_multi_slot_can_be_retrieved_without_providing_employee_id()
    {
        $emp = Employee::factory()->no_days_off()->create();
        $services = $this->makeServicesFor($emp, 2);
        $slots = $this->makeSlotsFor($emp, 2);
    ...
    /** @test */
    public function specifying_employee_id_will_return_available_multi_slots_for_only_that_particular_employee()
    {
    ...
    /** @test */
    public function a_multi_slot_only_counts_towards_consecutiveness_if_they_are_on_the_same_day()
    {
        $emp = Employee::factory()->no_days_off()->create();
        $services = $this->makeServicesFor($emp, 2);
        $slots = $this->makeSlotsFor($emp, 2);
    ...
    /** @test */
    public function a_multi_slot_only_counts_towards_consecutiveness_if_they_belong_to_the_same_employee()
    {
        $emp1 = Employee::factory()->no_days_off()->create();
        $emp2 = Employee::factory()->create(['company_id' => $emp1->company_id]);
        $services = $this->makeServicesFor([$emp1, $emp2]);
    ...
    /** @test */
    public function number_of_available_multi_slots_is_0_when_all_slots_are_reserved()
    {
        $emp = Employee::factory()->no_days_off()->create();
        $services = $this->makeServicesFor($emp, 2);
        $slots = $this->makeSlotsFor($emp, 2);
    ...
    /** @test */
    public function multi_slots_after_the_provided_date_to_will_not_be_retrieved()
    {
        $emp = Employee::factory()->no_days_off()->create();
        $services = $this->makeServicesFor($emp, 2);
        $slots = $this->makeSlotsFor($emp, 4);
        $slots[2]->update(['start_time' => $slots[2]->start_time->addMonths(12), 'end_time' => $slots[2]->end_time->addMonths(12)]);
        $slots[3]->update(['start_time' => $slots[3]->start_time->addMonths(12), 'end_time' => $slots[3]->end_time->addMonths(12)]);
        $url = $this->makeSlotsUrl($emp->company_id, $services->pluck('id'), $emp->id);
    ...
    /** @test */
    public function only_one_multi_slot_can_be_retrieved_per_start_time_when_employee_is_not_specified()
    {
        $emp1 = Employee::factory()->no_days_off()->create();
        $emp2 = Employee::factory()->create(['company_id' => $emp1->company_id]);
    ...
    /** @test */
    public function a_multi_slot_query_is_scoped_to_a_single_company()
    {
        $emp1 = Employee::factory()->no_days_off()->create();
    ...
    /** @test */
    public function a_multi_slot_will_not_be_retreived_if_its_start_time_has_passed()
    {
        $emp = Employee::factory()->no_days_off()->create();
        $services = $this->makeServicesFor($emp, 2);
        $slots = $this->makeSlotsFor($emp, 2);
    ...
    /** @test */
    public function a_multi_slot_will_not_be_retreived_if_now_plus_booking_grace_period_is_greater_than_its_start_time()
    {
        $emp = Employee::factory()->no_days_off()->create();
        $services = $this->makeServicesFor($emp, 2);
        $slots = $this->makeSlotsFor($emp, 2);
        $slots[0]->update(['start_time' => now(), 'end_time' => now()->addMinutes(14)]);
        $slots[1]->update(['start_time' => now()->addMinutes(14), 'end_time' => now()->addMinutes(29)]);
        $url = $this->makeSlotsUrl($emp->company_id, $services->pluck('id'), $emp->id);
    ...
    /** @test */
   public function a_multi_slot_will_not_be_retreived_if_the_requested_employee_is_not_working()
   {
        $emp = Employee::factory()->no_days_off()->create();
        $services = $this->makeServicesFor($emp, 2);
        $slots = $this->makeSlotsFor($emp, 2);
    ...
    /** @test */
    public function a_multi_time_slot_will_not_be_retreived_if_the_requested_employee_has_online_bookings_disabled()
    {
    ...
    /** @test */
    public function multi_slots_will_not_be_shown_for_employees_when_a_service_is_selected_that_the_employee_cannot_perform()
    {
        $emp1 = Employee::factory()->no_days_off()->create();
